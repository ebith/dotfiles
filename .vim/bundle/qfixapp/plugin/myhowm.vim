...
"  Last Modified: 2011-02-12 11:00
"        Version: 2.42
"=============================================================================
...
if !exists('g:QFixHowm_ListReminderCacheTime')
  let g:QFixHowm_ListReminderCacheTime = 3*60
endif
...
"初回保存時にファイル名を変更する
if !exists('g:QFixHowm_RenameAtFirstWrite')
  let g:QFixHowm_RenameAtFirstWrite = 0
endif
...
if g:QFixHowm_Default_Key > 0
  ...
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'A :call QFixHowmFileList<CR>'
  ...
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'t  :QFixHowmListReminderTodoCache<CR>'
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'y  :QFixHowmListReminderScheCache<CR>'
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'rt  :QFixHowmListReminderTodo<CR>'
  exec 'silent! nnoremap <unique> <silent> '.s:QFixHowm_Key.'ry  :QFixHowmListReminderSche<CR>'
...
if QFixHowm_MenuBar
  ...
  exec 'amenu <silent> 41.332 '.s:menu.'.DiaryList<Tab>  :call QFixHowmListDiary()<CR>'
  ...
  exec 'amenu <silent> 41.332 '.s:menu.'.ListAllAlt(&E)<Tab>'.s:QFixHowm_Key.'ra  :<C-u>call QFixHowmListAllTitleAlt("reload")<CR>'
  ...
  exec 'amenu <silent> 41.332 '.s:menu.'.Schedule(&Y)<Tab>'.s:QFixHowm_Key.'y  :<C-u>QFixHowmListReminderScheCache<CR>'
  exec 'amenu <silent> 41.332 '.s:menu.'.Todo(&T)<Tab>'.s:QFixHowm_Key.'t  :<C-u>QFixHowmListReminderTodoCache<CR>'
  exec 'amenu <silent> 41.332 '.s:menu.'.Rebuild-Schedule(&I)<Tab>'.s:QFixHowm_Key.'ry  :<C-u>QFixHowmListReminderSche<CR>'
  exec 'amenu <silent> 41.332 '.s:menu.'.Rebuild-Todo(&K)<Tab>'.s:QFixHowm_Key.'rt  :<C-u>QFixHowmListReminderTodo<CR>'
...
augroup QFixHowm
  autocmd!
  exec "autocmd FileType ".g:QFixHowm_FileType." call s:SetbuflocalSettings()"
...
function! QFixHowmListAllTitle(pattern, days, ...)
  if g:QFixHowm_TitleListCache
    ...
    call QFixHowmListAllTitle_(a:pattern, a:days)
  endif
endfunction
...
function! QFixHowmCreateNewFile(...)
  ...
  if a:0 > 0
    if a:1 == ''
      ...
      if pattern != ''
        let file = fnamemodify(escape(g:howm_dir, ' ') .'/'. strftime(g:howm_filename), ':h') .'/'
        let file = file. escape(pattern, ' ')
      endif
    else
      let file = escape(g:howm_dir, ' ') .'/'.a:1
    endif
    let ereg = g:QFixHowm_FileExt . '\|howm'
...
command! -count -nargs=* QFixHowmListReminderScheCache if count > 0|let g:QFixHowm_ShowSchedule = count|endif|call QFixHowmListReminderCache("schedule")
command! -count -nargs=* QFixHowmListReminderTodoCache if count > 0|let g:QFixHowm_ShowScheduleTodo = count|endif|call QFixHowmListReminderCache("todo")
...
function! QFixHowmListReminderCache(mode)
  ...
  if g:QFixHowm_ListReminderCacheTime > 0 && lt < g:QFixHowm_ListReminderCacheTime
    ...
    QFixCopen
    call cursor(1, 1)
    echo 'QFixHowm : Cached '.a:mode . '. ('.lt/60.' minutes before)'
...
function! QFixHowmSortReminder(sq, mode)
  ...
  let todayfname = g:howm_dir.'/'.g:QFixHowm_TodayFname
  ...
  let QFixHowmReminderTodayLine += 1
  let str = strftime('['.s:hts_date.']')
"  let text = g:QFixHowm_ShowTodayLineStr . ' ' . str . ' ' .g:QFixHowm_ShowTodayLineStr
...
"priorityソート関数
"
function! s:QFixCompareRev(v1, v2)
  return (a:v1.text > a:v2.text?1:-1)
endfunction
function! s:QFixCompareDefault(v1, v2)
  return (a:v1.defaultpriority >= a:v2.defaultpriority?1:-1)
endfunction
...
function! QFixHowmPairFile()
  ...
  let ext = g:QFixHowm_FileExt
  let file = expand('%:t').'.'.ext
...
" howm_dir内のファイルでMRUに登録するファイルの正規表現
if !exists('g:QFixHowm_MRU_regxp')
  let g:QFixHowm_MRU_regxp = '*'
endif
...
augroup QFixHowmMRU
  ...
  autocmd VimLeave * call <SID>QFixHowmMRUVimLeave()
augroup END

function! s:QFixHowmMRUVimLeave()
  if s:QFixHowm_Init
    call QFixHowmSaveMru(0)
    call QFixHowmSaveMru(2)
  endif
endfunction
...
function! QFixHowmSaveMru(write)
  ...
  let prevPath = getcwd()
  let prevPath = escape(prevPath, ' ')
  if IsQFixHowmFile('%', g:QFixHowm_MRU_regxp) == 0
    return
  endif
  let file = expand('%:p')
  let file = substitute(file, '\\', '/', 'g')
  ...
  let todayfname = g:howm_dir.'/'.g:QFixHowm_TodayFname
  if expand(file) == expand(todayfname)
    return
  endif
  let lnum = line('.')
  let text = getline('.')
...
"    silent exec 'lchdir ' . escape(g:QFix_SearchPath, ' ')
  ...
  if s:prevfname != a:file
    ...
    if bufloaded(tmpfile) "バッファが存在する場合
      ...
      if g:QFixHowm_ForceEncoding && (ext == g:QFixHowm_FileExt || ext == s:howmsuffix)
        silent! exec '0read ++enc='.g:howm_fileencoding.' ++ff='.g:howm_fileformat.' '.tmpfile
      else
        silent! exec '0read '.tmpfile
      endif
...
augroup QFixHowmBuffer
  ...
  exec "autocmd BufWritePost       *.".g:QFixHowm_FileExt." call s:QFixHowmBufWritePost_()|call QFixHowmBufWritePost()"
  if g:QFixHowm_FileExt != s:howmsuffix
    exec "autocmd BufReadPre         *.howm silent! call QFixHowmInit()"
    exec "autocmd BufReadPost        *.howm silent! call QFixHowmBufReadPost_()"
    exec "autocmd BufRead,BufNewFile *.howm setlocal filetype=".g:QFixHowm_FileType
    exec "autocmd BufWritePre        *.howm call QFixHowmInsertLastModified()|call QFixHowmBufWritePre()"
    exec "autocmd BufWritePost       *.howm call s:QFixHowmBufWritePost_()|call QFixHowmBufWritePost()"
  endif
augroup END
...
function! IsQFixHowmFile(file, ...)
  if a:file == '%' && &filetype =~ g:QFixHowm_FileType
    return 1
  endif
  let file = expand(a:file)
  ...
  if fnamemodify(file, ':p') =~ '^'.fnamemodify(expand(g:howm_dir), ':p')
    return 2
  endif
  ...
  for i in range(2, g:QFixHowm_howm_dir_Max)
    ...
    if fnamemodify(file, ':p') =~ '^'.fnamemodify(hdir, ':p')
      return 2
    endif
  endfor
  return 0
...
function! s:QFixHowmBufWritePost_()
  ...
  if s:QFixHowmFirstWrite && g:QFixHowm_RenameAtFirstWrite
    call QFixHowmRename_()
    doau BufRead
  endif
  let s:QFixHowmFirstWrite = 0
...
function! QFixHowmInsertLastModified(...)
  ...
  let s:QFixHowmFirstWrite = !filereadable(expand('%'))
  let saved_reg = @/
...
nnoremap <silent> g,rN :QFixHowmRenameAll<CR>

if !exists('g:QFixHowm_RenameAllExcludeRegxp')
  let g:QFixHowm_RenameAllExcludeRegxp = '000000'
endif
...
function! QFixHowmRenameAll(...)
  ...
  for d in list
    ...
    if from =~ g:QFixHowm_RenameAllExcludeRegxp.'\|'.'[/\\]'.g:QFixHowm_PairLinkDir .'[/\\]'
      continue
    endif
    let fname = substitute(d['text'], '^'.l:QFixHowm_Title . '\s*\|\.\+$', '', 'g')
    let fname = substitute(fname, '^\(\[[^\]]\+\]\s*\)\+', '', 'g')
    let fname = QFixHowmFormatFileName(fname)
    let from = substitute(from, '^'.expand(g:howm_dir).'.', '', '')
  ...
  nnoremap <buffer> ! :call QFixHowmRenameAll_()<CR>
